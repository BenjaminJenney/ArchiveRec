...
def search_messages(service, user_id, search_string):
    try:
        #this is basically the object access pattern for google api, need service object, users() for user inbox, messages from user inbox, do stuff: list, get, etc. 
...
service = get_service()
message_ids = search_messages(service, 'me', 'subject:confirmation')
get_message(service, 'me', message_ids[0])
